...
def test_not_non_conditional() -> None:
    req = cast(HEADER_TYPE, {})
    res = cast(HEADER_TYPE, {})
    assert not fresh(req, res)


def test_etags_match() -> None:
    req = cast(HEADER_TYPE, {"if-none-match": '"foo"'})
    res = cast(HEADER_TYPE, {"ETag": '"foo"'})
    assert fresh(req, res)


def test_etags_mismatch() -> None:
    req = cast(HEADER_TYPE, {"if-none-match": '"bar"'})
    res = cast(HEADER_TYPE, {"etag": '"foo"'})
    assert not fresh(req, res)


def test_etag_least_one_matches() -> None:
    req = cast(HEADER_TYPE, {"if-none-match": '"bar" , "foo"'})
    res = cast(HEADER_TYPE, {"ETag": '"foo"'})
    assert fresh(req, res)
...
def test_etag_exact() -> None:
    req = cast(HEADER_TYPE, {"if-none-match": 'W/"foo"'})
    res = cast(HEADER_TYPE, {"etag": 'W/"foo"'})
    assert fresh(req, res)
...
def test_none_match_not_macth() -> None:
    req = cast(HEADER_TYPE, {"if-none-match": '*'})
    res = cast(HEADER_TYPE, {"etag": '"bor"'})
    assert fresh(req, res)
...
def test_if_modified_since() -> None:
    ...
    req = cast(HEADER_TYPE, {"if-modified-since": 'Sat, 01 Jan 2000 00:00:00 GMT'})
    res = cast(HEADER_TYPE, {"last-modified": 'Sat, 01 Jan 2000 01:00:00 GMT'})
    assert not fresh(req, res)
...
def test_no_cache() -> None:
    req = cast(HEADER_TYPE, {'cache-control': ' no-cache'})
    res = cast(HEADER_TYPE, {})
    assert not fresh(req, res)
    # ETags match
